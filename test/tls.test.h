...
static void mock_tls_queue_outbound(mock_tls_ctx_t *m, ql_enc_level_t level,
...
static int mock_provide_data(void *ctx, ql_enc_level_t level,
                              const uint8_t *data, size_t len)
{
    mock_tls_ctx_t *m = (mock_tls_ctx_t *)ctx;
    m->provide_calls[level]++;
    if ((int)level == m->force_provide_fail_level) return -1;
    if (m->inlen[level] + len > MOCK_TLS_INBUF_CAP) return -1;
    if (len > 0) memcpy(m->inbuf[level] + m->inlen[level], data, len);
    m->inlen[level] += len;
    return 0;
}

static int mock_get_data(void *ctx, ql_enc_level_t level,
...
{
    mock_tls_ctx_t ctx; mock_tls_ctx_reset(&ctx);
    ql_tls_t tls; mock_tls_wire(&tls, &ctx);
...
TEST(test_tls_provide_data_propagates_engine_error)
{
    ...
    int rc = ql_tls_provide_data(&tls, QL_ENC_LEVEL_HANDSHAKE, bad_finished, sizeof(bad_finished) - 1);
...
TEST(test_tls_get_data_drains_across_multiple_small_reads)
{
    ...
    uint8_t flight[37];
    for (size_t i = 0; i < sizeof(flight); i++) flight[i] = (uint8_t)i;
...
TEST(test_tls_mock_handshake_end_to_end)
{
    ...
    mock_tls_queue_outbound(&cctx, QL_ENC_LEVEL_INITIAL,
                             client_hello, sizeof(client_hello) - 1);
    ...
    EXPECT_EQ(ql_tls_install_keys(&stls, QL_ENC_LEVEL_INITIAL,   &server_keys[QL_ENC_LEVEL_INITIAL]), 0);
    EXPECT_EQ(ql_tls_install_keys(&stls, QL_ENC_LEVEL_HANDSHAKE, &server_keys[QL_ENC_LEVEL_HANDSHAKE]), 0);

    /* Flight 2: server -> client, Handshake level */
    mock_tls_queue_outbound(&sctx, QL_ENC_LEVEL_HANDSHAKE,
                             server_flight, sizeof(server_flight) - 1);
    ...
    EXPECT_EQ(ql_tls_install_keys(&ctls, QL_ENC_LEVEL_INITIAL,   &client_keys[QL_ENC_LEVEL_INITIAL]), 0);
    EXPECT_EQ(ql_tls_install_keys(&ctls, QL_ENC_LEVEL_HANDSHAKE, &client_keys[QL_ENC_LEVEL_HANDSHAKE]), 0);
    ...
    mock_tls_queue_outbound(&cctx, QL_ENC_LEVEL_HANDSHAKE,
                             client_finished, sizeof(client_finished) - 1);
    ...
    EXPECT_EQ(cctx.inlen[QL_ENC_LEVEL_HANDSHAKE],
              sizeof(server_flight) - 1 + sizeof(client_finished) - 1 - (sizeof(client_finished) - 1));
...
static SSL_CTX *tls_test_make_server_ctx(void)
{
    ...
    X509 *cert = X509_new();
    if (!cert) { EVP_PKEY_free(pkey); return NULL; }
    ...
    X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC,
                                (const unsigned char *)"qlite-test", -1, -1, 0);
...
TEST(test_tls_real_handshake_end_to_end_via_quictls)
{
    ...
    while ((!ql_tls_handshake_done(&ctls) || !ql_tls_handshake_done(&stls))
           && progressed && rounds < 50) {
        progressed = false;
        for (int l = 0; l < QL_ENC_LEVEL_COUNT; l++) {
    ...
    for (int l = 0; l < QL_ENC_LEVEL_COUNT; l++) {
        if (l == QL_ENC_LEVEL_EARLY_DATA) continue;  /* no 0-RTT in this test */
    ...
    EXPECT_EQ(memcmp(ck[QL_ENC_LEVEL_APP].read.key,
                      sk[QL_ENC_LEVEL_APP].write.key,
                      ck[QL_ENC_LEVEL_APP].read.key_len), 0);
    EXPECT_NE(memcmp(ck[QL_ENC_LEVEL_APP].read.key,
                      ck[QL_ENC_LEVEL_APP].write.key,
                      ck[QL_ENC_LEVEL_APP].read.key_len), 0);
