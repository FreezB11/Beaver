...
static const ql_keys_t RFC_INITIAL_CLIENT_KEYS = {
    .key = {0x1f, 0x36, 0x96, 0x13, 0xdd, 0x76, 0xd6, 0x17, 0x8a, 0x84, 0x67, 0x43, 0x91, 0xc3,
            0x27, 0x25},
    .iv  = {0xfa, 0x04, 0x4b, 0x2f, 0x42, 0xa3, 0xfd, 0x3b, 0x46, 0xfb, 0x25, 0x5c},
    .hp = {0x9f, 0x50, 0x44, 0x9e, 0x04, 0xa0, 0xe8, 0x10, 0x28, 0x38, 0x3d, 0x57, 0x97, 0x73, 0x73,
           0x6a}, /* 0x73,0x6a */
    .key_len = 16,
    .iv_len  = 12,
    .hp_len  = 16,
...
TEST(test_aead_seal_basic) {
    ...
    int n = ql_aead_seal(&k, /*pkt_num=*/0, aad, sizeof(aad), pt, sizeof(pt), ct, sizeof(ct));
...
TEST(test_aead_seal_open_roundtrip) {
    ...
    uint8_t ct[sizeof(pt) + QL_AEAD_TAG_LEN];
    int sealed = ql_aead_seal(&k, 7, aad, sizeof(aad), pt, sizeof(pt), ct, sizeof(ct));
    EXPECT_GT(sealed, 0);

    uint8_t recovered[sizeof(pt)];
    int opened =
        ql_aead_open(&k, 7, aad, sizeof(aad), ct, (size_t)sealed, recovered, sizeof(recovered));
...
TEST(test_aead_tampered_ciphertext_rejected) {
    ...
    uint8_t ct[sizeof(pt) + QL_AEAD_TAG_LEN];
    int sealed = ql_aead_seal(&k, 42, aad, sizeof(aad), pt, sizeof(pt), ct, sizeof(ct));
    EXPECT_GT(sealed, 0);

    ct[0] ^= 0x01; /* flip one bit in the ciphertext body */

    uint8_t out[sizeof(pt)];
    int opened = ql_aead_open(&k, 42, aad, sizeof(aad), ct, (size_t)sealed, out, sizeof(out));
...
TEST(test_aead_tampered_tag_rejected) {
    ...
    uint8_t ct[sizeof(pt) + QL_AEAD_TAG_LEN];
    int sealed = ql_aead_seal(&k, 1, NULL, 0, pt, sizeof(pt), ct, sizeof(ct));
    EXPECT_GT(sealed, 0);
    ...
    uint8_t out[sizeof(pt)];
    int opened = ql_aead_open(&k, 1, NULL, 0, ct, (size_t)sealed, out, sizeof(out));
    EXPECT_LT(opened, 0);
}
...
TEST(test_aead_tampered_aad_rejected) {
    ...
    uint8_t ct[sizeof(pt) + QL_AEAD_TAG_LEN];
    int sealed = ql_aead_seal(&k, 99, aad, sizeof(aad), pt, sizeof(pt), ct, sizeof(ct));
    EXPECT_GT(sealed, 0);

    uint8_t out[sizeof(pt)];
    int opened =
        ql_aead_open(&k, 99, bad_aad, sizeof(bad_aad), ct, (size_t)sealed, out, sizeof(out));
...
TEST(test_aead_wrong_pkt_num_rejected) {
    ...
    int opened = ql_aead_open(&k, 11, NULL, 0, ct, sizeof(ct), out, sizeof(out));
    EXPECT_LT(opened, 0);
}
...
TEST(test_aead_256_roundtrip) {
    ...
    uint8_t ct[sizeof(pt) + QL_AEAD_TAG_LEN];
    int sealed = ql_aead_seal(&k, 256, aad, sizeof(aad), pt, sizeof(pt), ct, sizeof(ct));
    EXPECT_GT(sealed, 0);

    uint8_t out[sizeof(pt)];
    int opened = ql_aead_open(&k, 256, aad, sizeof(aad), ct, (size_t)sealed, out, sizeof(out));
    EXPECT_EQ(opened, (int)sizeof(pt));
...
TEST(test_aead_rfc9001_vectors) {
     ...
     */
    static const uint8_t aad[] = {0xc3, 0x00, 0x00, 0x00, 0x01, 0x08, 0x83, 0x94, 0xc8, 0xf0,
    ...
    int sealed =
        ql_aead_seal(&RFC_INITIAL_CLIENT_KEYS, 2, aad, sizeof(aad), pt, sizeof(pt), ct, sizeof(ct));
    EXPECT_EQ(sealed, (int)(sizeof(pt) + QL_AEAD_TAG_LEN));

    /* 2. open must recover the exact plaintext */
    uint8_t recovered[sizeof(pt)];
    int opened = ql_aead_open(&RFC_INITIAL_CLIENT_KEYS, 2, aad, sizeof(aad), ct, (size_t)sealed,
    ...
    int tampered = ql_aead_open(&RFC_INITIAL_CLIENT_KEYS, 2, aad, sizeof(aad), ct, (size_t)sealed,
                                bad, sizeof(bad));
...
TEST(test_hp_protect_remove_roundtrip_long) {
    ...
    const uint8_t sample[16] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
...
TEST(test_hp_protect_remove_roundtrip_short) {
    ...
    const uint8_t sample[16] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22,
...
TEST(test_hp_idempotent_double_remove) {
    ...
    const uint8_t sample[16] = {0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80,
                                0x90, 0xA0, 0xB0, 0xC0, 0xD0, 0xE0, 0xF0, 0x00};

    const uint8_t sample2[16] = {0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90,
...
TEST(test_hp_rfc9001_vectors) {
     ...
     *     0x00 ^ 0xf5 = 0xf5
     */
    uint8_t hdr[20]          = {0xc3, 0x00, 0x00, 0x00, 0x01, 0x08, 0x83, 0x94, 0xc8, 0xf0,
                                0x3e, 0x51, 0x57, 0x08, 0x00, 0x00, 0x44, 0x9e, 0x00, 0x00};
    const uint8_t sample[16] = {0xd1, 0xb1, 0xc9, 0x8d, 0xd7, 0x68, 0x9f, 0xb8,
