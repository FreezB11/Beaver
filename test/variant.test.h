...
TEST(test_decode_rfc_examples) {
    ...
    EXPECT_EQ(ql_varint_decode((uint8_t[]){0xC2, 0x19, 0x7C, 0x5E, 0xFF, 0x14, 0xE8, 0x8C}, 8, &v),
              8);
    EXPECT_EQ(v, 151288809941952652ULL);
}

TEST(test_boundary_values) {
    ql_varint_t values[] = {0,
                            1,
                            62,
                            63,
                            64,
...
TEST(test_pkt_num_encode_decode) {
     ...
     */
    decoded_pn = ql_pkt_num_decode(truncated_pn, pn_len * 8, 999);
...
TEST(test_pkt_num_decode_wrap) {
     ...
     */
    decoded_pn = ql_pkt_num_decode(0x00, 8, 255);
