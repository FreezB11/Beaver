...
TEST(test_frame_new_connection_id_roundtrip) {
    ...
    EXPECT(memcmp(dec.u.new_cid.cid.data, enc.u.new_cid.cid.data, 8) == 0);
    EXPECT(memcmp(dec.u.new_cid.stateless_reset_token.data,
                  enc.u.new_cid.stateless_reset_token.data, QL_RESET_TOKEN_LEN) == 0);
}
...
TEST(test_frame_connection_close_transport_roundtrip) {
    ...
    EXPECT(memcmp(dec.u.conn_close.reason_phrase, reason, sizeof(reason) - 1) == 0);
}
